        ...
        <div className="shortcut-list flex flex-col gap-2.5">
          {SHORTCUTS.map((s) => (
            <div className="shortcut-row flex items-center gap-3" key={s.key}>
              <kbd className="inline-flex items-center justify-center min-w-8 h-7 px-2 bg-bg-surface border border-border-default rounded-md font-['Archivo',sans-serif] text-[14px] font-semibold text-text-primary">
                {s.key}
              </kbd>
              <span className="text-text-secondary text-base">{s.description}</span>
            </div>
          ))}
        </div>
        <button
