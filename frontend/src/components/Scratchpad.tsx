...
export default function Scratchpad({ sessionName, onFocusTerminal }: ScratchpadProps) {
  ...
  // The `cancelled` flag prevents a stale fetch from a prior session from calling
  // setContent after the user has switched sessions, which would poison the new
  // session's localStorage draft (since setContent writes under the current storageKey).
  useEffect(() => {
    let cancelled = false
